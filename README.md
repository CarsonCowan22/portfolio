...
Deploy to Vercel with zero extra configuration. Push to GitHub, import the repo into Vercel, and deploy. No environment variables are required.

## Private tools

The job-hunt dashboard, budget tool, and coparent calendar that used to live in this repo (under
`/dashboard/job-hunt`, `/budget`, and `/coparent`) have moved to a separate private repo,
`carson-private-tools` -- personal tools handling real data don't belong in a repo meant to be a
public work sample. They're deployed independently and reachable from their own subdomains
(`budget.<domain>`, etc.); nothing about them lives here anymore.
